...
{
    if(CGSizeEqualToSize(imageShowSize, CGSizeZero))
    {
        imageShowSize = self.frame.size;
    }
    switch (self.arrangement) {
        ...
        case MButtonContentArrangementHorizontal:
        {
            ...
            if(self.imageAtEnd)
            {
              contentRect.origin.x= (self.frameWidth-(textSize.width+imageShowSize.width + spacing))/2.0;
            }
            else{
                contentRect.origin.x= spacing + (self.frameWidth-(textSize.width+imageShowSize.width + spacing))/2.0+ imageShowSize.width;
            }
            contentRect.origin.y =(self.frameHeight-textSize.height)/2.0;
        ...
        case MButtonContentArrangementVertical:
        {
            ...
            contentRect.origin.y =spacing + (self.frameHeight-(textSize.height+ imageShowSize.height+spacing))/2.0+ imageShowSize.height;
        }
            break;
        default:
            break;
...
- (CGRect)imageRectForContentRect:(CGRect)contentRect
{
  ...
  switch (self.arrangement) {
      ...
      case MButtonContentArrangementHorizontal:
      {
          ...
          if(self.imageAtEnd)
          {
              imageX = self.titleLabel.frame.origin.x+self.titleLabel.frameWidth+spacing;
         }
          else{
            imageX = (self.frameWidth-(self.titleLabel.frameWidth+imageShowSize.width+spacing))/2.0;
              
          }
          if(UIControlContentHorizontalAlignmentLeft == self.contentHorizontalAlignment)imageX=0;
          CGFloat imageY = (self.frameHeight-imageShowSize.height )/2.0;
      ...
      case MButtonContentArrangementVertical:
      {
          ...
          CGFloat imageX = (self.frameWidth-imageShowSize.width)/2.0;
          
          CGFloat imageY = (self.frameHeight-(self.titleLabel.frameHeight+ imageShowSize.height +spacing))/2.0;
...
- (void)setSelected:(BOOL)selected{
    ...
    isSelected = selected;
    self.backgroundColor = isSelected?selectColor:normallColor;
}

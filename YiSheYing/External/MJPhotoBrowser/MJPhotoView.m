...
- (void)hide
{
    ...
    UIImageView *srcView = _photo.srcImageView;
    if (self.photoViewDelegate && [self.photoViewDelegate respondsToSelector:@selector(srcImageViewForPhotoView:)]) {
        srcView = [self.photoViewDelegate srcImageViewForPhotoView:self];
    }
    
    // 清空底部的小图
    UIImage *curImage = srcView.image;
//    srcView.image = nil;
    
    CGFloat duration = 0.25;
    if (srcView.clipsToBounds) {
        [self performSelector:@selector(reset:) withObject:curImage afterDelay:duration];
    }
    
    [UIView animateWithDuration:duration + 0.1 animations:^{
        _imageView.frame = [srcView convertRect:srcView.bounds toView:nil];

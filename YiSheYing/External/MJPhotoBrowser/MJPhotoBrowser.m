...
- (void)viewDidLoad
{
    ...
    _bgImageView.backgroundColor = [[UIColor blackColor] colorWithAlphaComponent:1];
...
//    [UIApplication sharedApplication].statusBarHidden = _statusBarHiddenInited;
    ...
    [UIView animateWithDuration:0.35 animations:^{
        ...
        ws.view.backgroundColor = [UIColor clearColor];
        // 移除工具条
...
- (UIImageView *)srcImageViewForPhotoView:(MJPhotoView *)photoView{
    
    UIImageView *srcView = nil;
    NSInteger index = photoView.tag-kPhotoViewTagOffset;
    if (index<self.srcImageViews.count && index>=0) {
        srcView = self.srcImageViews[index];
    }
    return srcView;
}
...
- (void)loadImageNearIndex:(int)index
{
    if (index > 0) {
        ...
        [[SDWebImageDownloader sharedDownloader] downloadImageWithURL:photo.url options:SDWebImageLowPriority|SDWebImageRetryFailed progress:^(NSInteger receivedSize, NSInteger expectedSize) {

...
+ (NSString *)timeStringWithTimeStamp:(NSTimeInterval)timestamp withFormat:(NSString *)format{
    return [self timeStringFromTimeStamp:[NSString stringWithFormat:@"%f",timestamp] withFormat:format];
}
...
+ (NSDate *)endOfMonth:(NSDate *)date {
    ...
    NSDateComponents* parts = [[NSCalendar currentCalendar] components:flags fromDate:date];
    [parts setDay:numberOfDaysInMonth];
    [parts setHour:23];
    [parts setMinute:59];
    [parts setSecond:59];
    return [[NSCalendar currentCalendar] dateFromComponents:parts];

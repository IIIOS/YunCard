...
//    [self.view bringSubviewToFront:self.titleBarView];
    ...
    if (self.nodataInfoView) {
        self.nodataInfoView.frame = [self noInfoViewFrame];
    }
}
...
- (void)viewDidLoad {
  ...
  if ([self willCareKeyboard]) {
    ...
    //登录代理
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(didLogin) name:kAFNotiDidLogin object:nil];
...
//      self.navigationItem.backBarButtonItem = temporaryBarButtonItem;
        ...
        if(self.navigationController.viewControllers.count && self.navigationController.viewControllers[0] != self){
            UIButton *button = [[UIButton alloc] initWithFrame:CGRectMake(0, 0, 42, 42)];
            [button setImage:[UIImage imageNamed:@"btn_back_navi"] forState:UIControlStateNormal];
...
//            self.navigationItem.leftBarButtonItems = [NSArray arrayWithObjects:flexSpacer, leftButtonItem, nil];
   ...
   //----------NoInfoView
    if ([self showNoInfoView]) {
        self.nodataInfoView = [[NoDataInfoView alloc] initWithFrame:[self noInfoViewFrame]];
        self.nodataInfoView.hidden = YES;
        
        self.nodataInfoView.topOffset = self.nodataInfoView.frame.size.height/2-20;
        self.nodataInfoView.showItems = ShowNODataViewItemInfoLabel;
        self.nodataInfoView.userInteractionEnabled = NO;
        self.nodataInfoView.backgroundColor = [UIColor clearColor];
        [self.nodataInfoView.actionBtn addTarget:self action:@selector(noInfoActionButtonClicked:) forControlEvents:UIControlEventTouchUpInside];
        [self.view addSubview:self.nodataInfoView];
    }
...
- (UIViewController *)pushViewController:(NSString *)className
  ...
  if ([pushedViewController isKindOfClass:[BaseViewController class]]) {
    ...
    //Add by helfy  判断是否存在navigationController。不存在则使用appDelegate 的rootNav来push
  ...
  pushedViewController.hidesBottomBarWhenPushed = YES;
    ...
    if (self.navigationController) {
        ...
        [[(YCAppDelegate *)[[UIApplication sharedApplication] delegate] rootNavgation] pushViewController:pushedViewController animated:YES];
    }
...
- (UIViewController *)popViewController {
  if (self.navigationController && [self.navigationController.viewControllers firstObject]!=self) { //如果有navigationBar
    NSInteger index =
 ...
 *  是否显示空数据占位图 默认为No
 *
 *  @return YES 表示需要显示NoInfoView  NO表示不显示NoInfoView
 */
- (BOOL)showNoInfoView{
    return NO;
}
